...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 17 17:11:34 2024
...
# Train Function
# ========================
def train_and_evaluate(model, train_loader, vali_loader, model_name, save_path, epochs=10, learning_rate=0.001):
    ...
    for ep in range(epochs):
        ...
        for x, y in train_loader:
            ...
            l2.backward()
            optimizer.step()
            train_l2_epoch += l2.item()
        train_l2_epoch /= len(train_loader)
        train_l2.append(train_l2_epoch)
        ...
        with torch.no_grad():
            for x, y in vali_loader:
                x, y = x.cuda(), y.cuda()
                out = model(x)
                vali_l2_epoch += myloss(out.view(batch_size, -1), y.view(batch_size, -1)).item()
        vali_l2_epoch /= len(vali_loader)
        vali_l2.append(vali_l2_epoch)
        print(f"Epoch {ep+1}, Train L2: {train_l2_epoch:.6f}, Vali L2: {vali_l2_epoch:.6f}")

    # Save the results
    scipy.io.savemat(f"{save_path}/{model_name}.mat", {"train_l2": train_l2, "vali_l2": vali_l2})


# ========================
# Run Models for Up to 5 Layers
# ========================
save_path = "Results"
os.makedirs(save_path, exist_ok=True)

for num_layers in range(1, 6):  # Up to 5 layers
    for model_type in ['FNO', 'LNO', 'FLNO']:
        print(f"\nTraining {model_type} Model with {num_layers} Layer(s)")
        model = CombinedModel(1, width, modes1_fno, modes2_fno, modes1_lno, modes2_lno, num_layers=num_layers, model_type=model_type).cuda()
        model_name = f"{model_type}_layers_{num_layers}"
        train_and_evaluate(model, train_loader, vali_loader, model_name, save_path, epochs=30, learning_rate=0.001)
