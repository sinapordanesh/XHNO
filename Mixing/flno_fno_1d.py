...
# ====================================
# FLNO Model Without Laplace Transform
# ====================================
class FLNO1d(nn.Module):
    def __init__(self, modes, width, num_layers):
        """
        A model with only SpectralConv1d and Conv1d layers. No Laplace Transform.
        """
        ...
        # Define SpectralConv1d and Conv1d layers dynamically
    ...
    def forward(self, x):
        ...
        # Apply SpectralConv1d and Conv1d layers
...
# Storage for results
results = {"train_loss": [], "vali_loss": [], "num_layers": []}

# Run for 1 to 10 layers
for num_layers in range(1, 11):
    print(f"Running model WITHOUT Laplace Transform with {num_layers} layers...")
    ...
    for ep in range(epochs):
        ...
        with torch.no_grad():
            val_loss = np.mean([
                F.mse_loss(model(x.cuda()).view(batch_size_vali, -1), y.cuda().view(batch_size_vali, -1)).item()
                for x, y in vali_loader
            ])
        vali_loss.append(val_loss)
        scheduler.step()

    print(f"Finished {num_layers} layers. Final Validation Loss: {vali_loss[-1]:.6f}")
    results["train_loss"].append(train_loss)
    results["vali_loss"].append(vali_loss)
    results["num_layers"].append(num_layers)

# Save results to a single .mat file
output_file = "training_results_no_laplace.mat"
