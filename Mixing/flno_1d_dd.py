...
class SpectralConv1d(nn.Module):
    ...
    def forward(self, x):
        x_ft = torch.fft.rfft(x)  # FFT of the input
        ...
        return torch.fft.irfft(out_ft, n=x.size(-1))  # Return to time domain
...
# ====================================
# FLNO Model with PR Applied to First Layer
# ====================================
class FLNO1d(nn.Module):
    def __init__(self, modes, width, num_layers):
        ...
        # PR for the first layer only
        self.pr_layer = PR(self.width, self.width, self.modes1)
    ...
    def forward(self, x):
        ...
        # Apply PR only to the first layer
        x = F.gelu(self.pr_layer(x) + self.fconv_layers[0](x) + self.w_layers[0](x))

        # Remaining layers: Fourier and Conv only
...
# Save Results
np.save(save_results_to + f'train_loss_{num_layers}Layers.npy', train_loss)
np.save(save_results_to + f'vali_loss_{num_layers}Layers.npy', vali_loss)
...
plt.savefig(save_results_to + f'loss_history_{num_layers}Layers.png')
plt.show()
